...
public:
  ...
  MessengerServer(boost::asio::io_context &ioContext,
                  const std::string &filename)
      : ioContext(ioContext), signal_(ioContext, SIGCHLD), acceptor(ioContext),
        socket_(ioContext) {
    ...
    if (setsockopt(acceptor.native_handle(), SOL_SOCKET,
...
private:
  ...
  void startAccept() {
    acceptor.async_accept(
        socket_, boost::bind(&MessengerServer::handleAccept, this, _1));
  }
  ...
  void handleAccept(const boost::system::error_code &ec) {
    if (!ec) {
      ...
      logger.log(WARNING, __FILE__, __LINE__, "Accept error: %s",
                 ec.message().c_str());
      startAccept();
    }
  }
