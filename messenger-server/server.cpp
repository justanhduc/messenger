...
void MessengerServer::startProcess() {
    ...
    switch (arg.action) {
        ...
        case TASK_SPOOLER: {
            ...
            acceptor.close();
            if (true) {
                int result = bp::system(tsCmd,
                                        fd.bind(1, socket_.native_handle()),
                                        fd.bind(2, socket_.native_handle()),
                                        bp::start_dir(arg.currentDir),
                                        arg.env.env
                );
...
//                execvp(tsCmd[0], tsCmd.data());
                ...
                int result = bp::system(tsCmd, bp::std_out.close(), fd.bind(2, socket_.native_handle()));
                if (result != 0)
                    std::cerr << "Command failed to execute" << std::endl;

                result = bp::system("ts -q",
                                    fd.bind(1, socket_.native_handle()),
                                    fd.bind(2, socket_.native_handle()));
                if (result != 0)
                    std::cerr << "Command failed to execute" << std::endl;
