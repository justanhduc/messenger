...
ints selectFreeGpus(int n) {
  ...
  if (gpuList.size() >= n)
    ...
    logger.log(ERROR, __FILE__, __LINE__,
               "There are %d free GPUs but requested %d", gpuList.size(), n);
  return freeGpus;
}
...
void showFreeGpuInfo() {
  ...
  for (auto it : freeGpuList) {
    ...
    if (result != 0) {
      logger.log(WARNING, __FILE__, __LINE__,
                 "Failed to get GPU handle for GPU %d: %s", it,
                 nvmlErrorString(result));
      goto Error;
    }
    ...
    if (result != 0) {
      logger.log(WARNING, __FILE__, __LINE__,
                 "Failed to get GPU memory for GPU %d: %s", it,
                 nvmlErrorString(result));
      goto Error;
    }
    ...
    std::cout << it << std::setw(24) << name << std::setw(13)
              << mem.total / (1024. * 1024 * 1024) << std::setw(21)
              << mem.free / (1024. * 1024 * 1024) << std::endl;
  }
